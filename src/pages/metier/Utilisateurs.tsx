...
const REGIONS: Region[] = [
  "Nord", "Nord-Est", "Nord-Ouest", "Ile-de-France", "Centre", "Sud-Est", "Sud-Ouest",
]
...
const FORM_INITIAL: DrawerForm = {
  prenom: "", nom: "", email: "", role: "consultant", region: "", password: "", is_active: true,
}

function roleLabel(role: string): string {
  const map: Record<string, string> = {
    admin: "Admin national", admin_national: "Admin national",
    responsable_regional: "Resp. régional", consultant: "Consultant",
  }
...
export default function Utilisateurs() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (profil) setCurrentUser({ id: user.id, role: profil.role, region: profil.region })
    }
    init()
  }, [])
  ...
  async function chargerUtilisateurs() {
    setLoading(true)
    try {
      let query = supabase.from("profils").select("id, prenom, nom, role, region")
        .in("role", ["admin", "admin_national", "responsable_regional", "consultant"])

      if (currentUser?.role === "responsable_regional" && currentUser.region) {
        query = query.eq("region", currentUser.region).eq("role", "consultant")
      }
      ...
      const mapped: Utilisateur[] = (data || []).map((p: any) => ({
        ...
        role: p.role, region: p.region, is_active: true,
      }))

      if (currentUser?.role === "admin" || currentUser?.role === "admin_national") {
        const ids = mapped.map(u => u.id)
        try {
          const { data: rpcData } = await supabase.rpc("get_users_emails", { user_ids: ids })
          rpcData?.forEach((au: { id: string; email: string }) => {
  ...
  async function toggleActif(u: Utilisateur) {
    ...
    setUtilisateurs(prev => prev.map(x => x.id === u.id ? { ...x, is_active: nouvelle_valeur } : x))
    try {
      await supabase.from("profils").update({ is_active: nouvelle_valeur } as any).eq("id", u.id)
    } catch {
      setUtilisateurs(prev => prev.map(x => x.id === u.id ? { ...x, is_active: u.is_active } : x))
    }
  }
  ...
  const utilisateursFiltres = utilisateurs.filter(u => {
    ...
    const matchRegion = filtreRegions.length === 0 ||
  ...
  async function handleCreer(e: React.FormEvent) {
    ...
    if ((form.role === "responsable_regional" || form.role === "consultant") && !form.region) {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-user`,
        {
          ...
          body: JSON.stringify({ email: form.email, password: form.password, prenom: form.prenom, nom: form.nom, role: form.role, region: form.region || null }),
        }
      )
      ...
      chargerUtilisateurs()
      setTimeout(() => { setDrawerOpen(false); setFormSuccess("") }, 1500)
    } catch (err: any) {
  ...
  async function handleEditer(e: React.FormEvent) {
    ...
    if ((editForm.role === "responsable_regional" || editForm.role === "consultant") && !editForm.region) {
    ...
    try {
      ...
      chargerUtilisateurs()
      setTimeout(() => { setDrawerEditOpen(false); setEditSuccess("") }, 1500)
    } catch (err: any) {
  ...
  return (
    <div className="page-wrapper">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        ...
        {isAdmin && (
          <button className="btn-primary" onClick={() => { setDrawerOpen(true); setForm(FORM_INITIAL); setFormError(""); setFormSuccess("") }}>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: "1", minWidth: "200px" }}>
          ...
          <input className="input" style={{ paddingLeft: "32px" }} placeholder="Rechercher un utilisateur…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        {isAdmin && (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {utilisateursFiltres.map(u => (
                <tr key={u.id} style={{ borderBottom: "1px solid #E2DDD8", height: "52px" }}
                ...
                >
                  ...
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", justifyContent: "flex-end" }}>
                      {isAdmin && (
                        <>
                          <button title="Modifier" style={actionBtnStyle} onClick={() => {
                            setEditUser(u)
                            setEditForm({ prenom: u.prenom, nom: u.nom, email: u.email, role: u.role as RoleAGE, region: (u.region as Region) || "", is_active: u.is_active })
                            setEditError("")
                            setEditSuccess("")
                          ...
                          <button title={u.is_active ? "Désactiver" : "Activer"}
            ...
            <form onSubmit={handleCreer} style={{ flex: 1, overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
              {formError && (
                <div style={{ padding: "10px 14px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "8px", fontSize: "13px", color: "#B91C1C", display: "flex", gap: "8px", alignItems: "center" }}>
                  <i className="ti ti-alert-triangle" style={{ fontSize: "14px", flexShrink: 0 }} />{formError}
                </div>
              )}
              {formSuccess && (
                <div style={{ padding: "10px 14px", background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: "8px", fontSize: "13px", color: "#2F7D5C", display: "flex", gap: "8px", alignItems: "center" }}>
                  <i className="ti ti-circle-check" style={{ fontSize: "14px", flexShrink: 0 }} />{formSuccess}
                </div>
              )}
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "6px" }}>
                  {([
                    ...
                    <label key={r.value} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "10px 12px", borderRadius: "8px", cursor: "pointer", border: `1px solid ${form.role === r.value ? "#B25C2A" : "#E2DDD8"}`, background: form.role === r.value ? "#F9F0EA" : "#FFFFFF" }}>
        ...
        <>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, height: "100vh", width: "400px", background: "#FFFFFF", zIndex: 400, display: "flex", flexDirection: "column", boxShadow: "-4px 0 24px rgba(0,0,0,0.12)" }}>
            ...
            <form onSubmit={handleEditer} style={{ flex: 1, overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
              {editError && (
                <div style={{ padding: "10px 14px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "8px", fontSize: "13px", color: "#B91C1C", display: "flex", gap: "8px", alignItems: "center" }}>
                  <i className="ti ti-alert-triangle" style={{ fontSize: "14px", flexShrink: 0 }} />{editError}
                </div>
              )}
              {editSuccess && (
                <div style={{ padding: "10px 14px", background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: "8px", fontSize: "13px", color: "#2F7D5C", display: "flex", gap: "8px", alignItems: "center" }}>
                  <i className="ti ti-circle-check" style={{ fontSize: "14px", flexShrink: 0 }} />{editSuccess}
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <div>
              ...
              <div>
                ...
                <input className="input" type="email" value={editForm.email || ""} onChange={e => setEditForm({ ...editForm, email: e.target.value })} placeholder="prenom.nom@agelegacy-qc.com" />
              </div>
              <div>
                <label style={labelStyle}>Rôle <span style={{ color: "#B91C1C" }}>*</span></label>
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "6px" }}>
                  {([
                    ...
                    <label key={r.value} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "10px 12px", borderRadius: "8px", cursor: "pointer", border: `1px solid ${editForm.role === r.value ? "#B25C2A" : "#E2DDD8"}`, background: editForm.role === r.value ? "#F9F0EA" : "#FFFFFF" }}>
              ...
              <div style={{ marginTop: "8px", padding: "16px", background: "#FEF2F2", borderRadius: "8px", border: "1px solid #FECACA" }}>
                ...
                <button type="button" onClick={handleSupprimer} disabled={editLoading} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 14px", borderRadius: "7px", fontSize: "13px", fontWeight: 500, cursor: "pointer", border: "1px solid #FECACA", background: "#FFFFFF", color: "#B91C1C", fontFamily: "inherit" }}>
                  <i className="ti ti-trash" style={{ fontSize: "14px" }} /> Supprimer cet utilisateur
                </button>
              </div>
            </form>
            <div style={{ padding: "16px 24px", borderTop: "1px solid #E2DDD8", display: "flex", gap: "8px", flexShrink: 0 }}>
