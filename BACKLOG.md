...
**Rapport de référence :** Session 17/07/2026 — P3-33 à P3-37 clos (audit complet isolation multi-tenant espace client, déclenché par la découverte que la GED métier n'avait jamais enregistré aucun document en production depuis P2-08 — colonnes de versioning manquantes en base + erreurs Supabase jamais lues côté code, échec silencieux). Pont créé entre GED métier et fiche actif client (`visible_client`), policy RLS ajoutée sur `risk_scores` pour le client, policies `actifs_documents`/`actifs_reglementaire` complétées. BUG-15 clos — diagnostic terminé, comportement déjà correct, signalement obsolète. Session 16/07/2026 (suite) — P3-31 et P3-32 clos (onglet Rapports dans Missions + notifications temps réel demandes/disponibilité de rapports). Session 16/07/2026 — P3-27 clos (Espace particulier : nouveau module indépendant Carnet d'Information du Logement — conformité art. L.126-33-1 CCH, fiche technique + historique travaux + export PDF, hook DPE prévu pour intégration future — voir détail ci-dessous). Session précédente 09/07/2026 — P3-26 en cours (AGEcarbon : rapport IA généré depuis la page Résultats, PDF multi-pages avec graphiques Recharts rasterisés, structure dynamique par poste réel du bilan — non testé de bout en bout, pas encore clos). Session 08/07/2026 — P3-25 clos (AGEcarbon : import données monétaires, reclassement poste/scope sur référentiel partagé, ajout de ligne manuelle, correctif calcul `abc_resultats` absent, correctif filtre montant=0, correctif désync poste saisie/facteur, policies RLS `abc_facteurs_emission`).
...
| P3-27 | Espace particulier | **Carnet d'Information du Logement (CIL)** — module indépendant, conforme art. L.126-33-1 CCH (décret n°2022-1674, arrêté 27/12/2022) : fiche technique du logement, historique des travaux à incidence énergétique, export PDF. Hook `dpe_document_id` prévu pour intégration future d'un module DPE (non existant à ce jour) — voir détail ci-dessous | `[x]` 16/07/2026 |
| BUG-13 | Messagerie | **Envoi de message AGE → client particulier échouait silencieusement** — `handleEnvoyer` (métier) écrivait `type_conversation: onglet` avec la valeur `"clients"` (pluriel) au lieu de `"client"` (singulier attendu par la contrainte CHECK `messages_type_conversation_check`). Insert rejeté sans remontée d'erreur côté UI (`.insert()` sans gestion d'erreur). Corrigé : normalisation `onglet === "clients" ? "client" : "interne"` + ajout de la gestion d'erreur (`alert` + `console.error`) sur l'insert | `[x]` 16/07/2026 |
| P3-28 | Espace particulier | **Notification messagerie sans contexte** — le badge sidebar (`Layout.tsx`) n'affichait qu'un total agrégé de messages non lus, sans indiquer l'onglet concerné (Demandes/Campagnes/Actifs). Corrigé : info-bulle (`title`) sur le badge avec répartition par catégorie + ouverture automatique de l'onglet le plus chargé en non-lus à l'arrivée sur `ClientMessagerie.tsx` | `[x]` 16/07/2026 |
| P3-29 | Espace particulier | **Page « Aides & Subventions » créée** — route `/client/aides` référencée depuis plusieurs mois (roadmap Accueil, raccourcis profil) sans jamais avoir été implémentée ni routée dans `App.tsx` → page blanche au clic. Page statique informative créée (`AidesSubventions.tsx`) : 3 dispositifs (CEE 6ᵉ période, Fonds de Prévention Argile RGA, MaPrimeRénov'/ANAH), conditions d'éligibilité résumées, règles de cumul, liens vers sites officiels. Contenu vérifié juillet 2026, pas de connexion aux données `dossiers_rga` (portée non demandée) | `[x]` 16/07/2026 |
| P3-30 | Espace particulier | **Onglet Reporting inadapté au particulier** — la page `Reporting.tsx` proposait un workflow de demande de rapports B2B (Bilan GES, CSRD, Bilan Carbone, Brown Value — types verrouillés par contrainte CHECK `rapports_client_type_rapport_check`) avec délai de traitement 48h, sans pertinence pour un particulier. Créé `ReportingParticulier.tsx` : détection `type_client === "proprietaire"` dans `Reporting.tsx`, page dédiée sans workflow de demande — 3 blocs à génération/accès instantané : Diagnostic climatique du bien (export PDF enrichi — plan de situation IGN Géoplateforme avec échelle et rose des vents, jauge de score 4 classes, détail des 6 aléas Géorisques réels via `georisques_data.risquesNaturels`, recommandations par classe de risque avec disclaimer), Dossier RGA (statut en direct + lien vers `DossierRGA.tsx`, ou lien Fonds de Prévention Argile si aucun dossier), Carnet d'Information du Logement (lien vers le module P3-27). Aucune modification de la contrainte CHECK ni de la table `rapports_client` — les types B2B restent inchangés pour les autres profils | `[x]` 16/07/2026 |
| BUG-14 | Espace client — Mon profil | **Sauvegarde silencieusement défaillante + mauvaise table cible** — `MonProfil.tsx` écrivait `prenom`/`nom`/`telephone`/`poste` dans la table `profils` (réservée à l'espace métier AGE) au lieu de `profils_client`, reproduisant le bug de routage déjà corrigé une fois (un client avec une ligne dans `profils` est basculé vers l'espace métier par `Layout.tsx`). Aucune gestion d'erreur sur les appels Supabase : le message de succès s'affichait même en cas d'échec silencieux. Le sélecteur "Type de structure" utilisait l'id `"particulier"` au lieu de `"proprietaire"` (valeur réelle en base), risquant de casser le routage/KPI/Reporting particulier si un utilisateur resélectionnait son profil. Corrigé : ajout des colonnes `telephone`/`poste` sur `profils_client` (migration), lecture/écriture entièrement redirigées vers `profils_client` (identité) + `organisations` (raison sociale/adresse si `organisation_id` existe), gestion d'erreur explicite avec message affiché, correction de l'id `"proprietaire"` | `[x]` 16/07/2026 |
| P3-31 | Missions (métier) | **Onglet "Rapports" — traitement des demandes de rapport client** — les demandes insérées dans `rapports_client` (statut `demande`) n'avaient aucune interface de traitement côté AGE malgré le plan validé en session de mai (jamais implémenté). Ajouté : onglet "Rapports" dans `Missions.tsx` (visible admin national + responsable régional, filtré par région pour ce dernier via jointure `profils_client.region`), liste avec identité client résolue (raison sociale via `organisations`, ou nom pour un particulier), drawer de traitement — changement de statut (`demande`/`en_cours`/`disponible`), upload PDF vers bucket privé `rapports-client` (policies : AGE admin/resp. régional en écriture, client propriétaire en lecture seule), saisie de KPIs dynamiques (paires clé/valeur en JSONB). Correctif associé sur `ClientReporting.tsx` : téléchargement via URL signée (`createSignedUrl`) au lieu d'un `href` direct, incompatible avec un bucket privé | `[x]` 16/07/2026 |
| P3-32 | Notifications | **Notifications temps réel demandes/disponibilité de rapports** — aucune notification n'existait ni à la soumission d'une demande de rapport (côté métier) ni à sa mise à disposition (côté client) ; les badges étaient calculés une seule fois au chargement de page sans rafraîchissement. Ajouté : colonne `rapports_client.vu_client` (migration), abonnements Supabase Realtime (`postgres_changes`) sur `rapports_client` — badge "File d'attente" + badge dédié "Missions" côté admin national et responsable régional (filtré par région à l'INSERT via jointure `profils_client`) s'incrémentent en direct ; badge "Reporting" côté client s'incrémente en direct à la mise à "disponible" et se réinitialise (`vu_client=true`) à l'ouverture de la page | `[x]` 16/07/2026 |
| BUG-15 | Espace client — Mes Demandes / Reporting | ~~Formulaire "Demander un rapport" propose tous les actifs, pas seulement ceux du client~~ — **fichier à jour analysé (`Reporting.tsx`) : filtre déjà correct** (`.eq("user_id", user.id).eq("categorie", "patrimoine_propre")`), confirmé comme comportement voulu par le PO (seul le patrimoine propre est demandable en rapport, pas `biens_finances` ni `import_csv`). Signalement probablement lié à une version antérieure du fichier, déjà corrigée avant ce diagnostic | `[x]` 17/07/2026 — reclassé non-reproductible |
| P3-33 | GED (M11) | **Documents GED jamais enregistrés depuis P2-08** — la table `documents` n'avait jamais reçu la migration de versioning (colonnes `version`, `document_parent_id`, `est_version_courante`, `region_code`, `note`, `bien_id` absentes), alors que `GED.tsx` les utilisait depuis sa mise en prod (29/06/2026). Chaque insert échouait en 400 (`PGRST204`), silencieusement — ni `handleUpload()` ni `loadDocuments()` ne lisaient `error` sur les appels Supabase, un toast "succès" s'affichait malgré l'échec total. **Aucun document n'a donc jamais été réellement enregistré en GED depuis le lancement de cette fonctionnalité** — voir détail ci-dessous | `[x]` 17/07/2026 |
| P3-34 | GED (M11) | **`documents.client_id` jamais renseigné à l'upload** — le champ existe en base et est utilisé par la policy RLS `documents_consultant_own`, mais `handleUpload()` ne le remplissait jamais. Corrigé : dérivation automatique depuis `actifs.client_id` (chaîne confirmée : `documents.client_id → profils_client.id → auth.users.id ← actifs.client_id`) lorsqu'un actif est sélectionné à l'upload | `[x]` 17/07/2026 |
| P3-35 | GED (M11) ↔ Espace client | **Pont GED métier → Fiche actif client** — la GED métier (`documents`) et les documents visibles côté client sur la fiche actif (`actifs_documents`) sont deux systèmes indépendants sans passerelle : un document uploadé par un consultant n'était jamais visible côté client. Architecture d'origine confirmée volontaire (`actifs_documents` = dépôts du client lui-même via le wizard de création d'actif), mais absence de tout mécanisme de partage consultant → client. Ajouté : colonne `documents.visible_client BOOLEAN DEFAULT false`, case à cocher optionnelle à l'upload GED (visible uniquement si un actif est sélectionné, décochée par défaut — sécurité par défaut), `FicheActif.tsx` interroge désormais aussi `documents` (filtré `actif_id` + `visible_client=true` + `est_version_courante=true`) en plus de `actifs_documents`, fusion à l'affichage avec libellé « Envoyé par AGE » — voir détail ci-dessous | `[x]` 17/07/2026 |
| P3-36 | Espace client | **`risk_scores` — aucune policy SELECT pour le client** — `ScoreHistorique.tsx` en `modeClient={true}` (affiché sur `FicheActif.tsx`, onglet Synthèse) interroge directement `risk_scores`, mais aucune policy RLS n'autorisait un compte client à lire ses propres scores (uniquement `admin`/`admin_national`/`responsable_regional`/`consultant`). Résultat : encart "Score climatique" affichait systématiquement "Score non encore calculé" côté client, même quand des scores existaient réellement — échec silencieux (RLS filtre sans erreur). Corrigé : policy `risk_scores_client_select` ajoutée (`actif_id IN (SELECT id FROM actifs WHERE client_id = auth.uid() OR user_id = auth.uid())`) | `[x]` 17/07/2026 |
| P3-37 | Espace client | **Audit isolation multi-tenant espace client — policies RLS complétées** — `actifs_documents` et `actifs_reglementaire` ne vérifiaient que `actifs.user_id = auth.uid()`, jamais `client_id` (contrairement à `actifs` elle-même qui vérifie les deux). Pas une fuite (les deux colonnes ne divergent jamais en pratique sur les 98 actifs existants), mais un risque d'accès bloqué pour les actifs n'ayant que `client_id` renseigné. Policies `users_own_documents` et `users_own_reglementaire` recréées pour couvrir `client_id OR user_id` — voir détail ci-dessous | `[x]` 17/07/2026 |

### Détail P3-33 à P3-37 — GED, pont client, audit isolation multi-tenant (clos)

**Contexte :** demande initiale du PO de valider que les documents attribués via la GED métier arrivent bien dans l'environnement client/actif concerné. Le diagnostic a révélé que la GED n'avait jamais fonctionné en production, ce qui a conduit à un audit systématique de l'isolation des données côté espace client.

**Diagnostic GED (P3-33/P3-34) :**
- Erreur observée : `POST .../documents 400` → `PGRST204 : Could not find the 'document_parent_id' column`.
- Cause racine : composant `GED.tsx` (P2-08, marqué clos le 29/06/2026) écrit avec un schéma jamais vérifié contre la base réelle — colonnes de versioning absentes en base.
- Colonnes ajoutées par migration : `version INT DEFAULT 1`, `document_parent_id UUID REFERENCES documents(id)`, `est_version_courante BOOLEAN DEFAULT true`, `region_code TEXT`, `note TEXT`, `bien_id UUID`, `visible_client BOOLEAN DEFAULT false`.
- `handleUpload()` et `loadDocuments()` corrigés pour lire et remonter les erreurs Supabase (`error`) au lieu de les ignorer silencieusement.
- FK `documents.actif_id → actifs.id` et policies RLS (`documents_admin_all`, `documents_consultant_own`) déjà présentes et correctes — non touchées.

**Pont GED → client (P3-35) :**
- Chaîne de résolution client confirmée : `documents.client_id → profils_client.id → auth.users.id ← actifs.client_id`. `profils_client.id` est directement `auth.users.id` (pas d'UUID séparé).
- `actifs.client_id` et `actifs.user_id` coexistent (98 vs 95 lignes renseignées, 0 divergence constatée) — `client_id` retenue comme colonne de référence, `user_id` traitée comme reliquat partiel à ne pas généraliser davantage.
- Documents GED marqués `visible_client=true` affichés côté client avec le libellé « Envoyé par AGE », distincts des dépôts du client lui-même (`actifs_documents`, wizard de création d'actif) — pas de fusion de table, deux sources interrogées et affichées ensemble.

**Audit isolation (P3-36/P3-37) — tables vérifiées :** `actifs`, `documents`, `actifs_documents`, `actifs_reglementaire`, `rapports_client`, `demandes_marketplace`, `risk_scores`, `contacts_campagne`.
- Déjà correctement isolées sans modification : `actifs`, `documents` (après P3-34), `rapports_client`, `demandes_marketplace`, `Reporting.tsx` (cf. clôture BUG-15 ci-dessus).
- Corrigées : `risk_scores` (policy SELECT client absente → ajoutée), `actifs_documents`/`actifs_reglementaire` (policies étendues à `client_id OR user_id`).

**Non traité dans cette session (à la demande du PO) :** nettoyage des données de test mélangées entre clients/actifs/profils réels — reporté, pas de risque identifié pour les nouveaux clients tant que P3-33 à P3-37 sont en place.

**Validé par le PO le 17/07/2026** — test manuel : upload GED avec case "Visible par le client" cochée, actualisation de la fiche actif côté client, document visible avec le bon libellé.

### Détail P3-27 — Carnet d'Information du Logement (clos)

**Fait le 16/07/2026 :**
- `CarnetLogement.tsx` créé (`src/pages/client/`) — module indépendant, accessible uniquement depuis l'espace client particulier (nouvelle entrée sidebar « Carnet du logement », visible uniquement si `labelProfil === "Particulier"`).
- Route `/client/carnet-logement` (sans `:id`) — le composant charge automatiquement le(s) bien(s) du particulier via `actifs.user_id = auth.uid()`, sélection auto si un seul bien, sélecteur simple si plusieurs.
- Tables créées : `carnet_logement_infos` (1 ligne par `actif_id`, `UNIQUE`), `carnet_logement_travaux` (N lignes, FK vers `carnet_logement_infos.id`). RLS : le particulier accède uniquement au carnet de ses propres biens (via `actifs.user_id`), lecture seule pour `admin`/`admin_national`.
- Bucket Storage `documents-carnet-logement` (privé), policies calquées sur le pattern `documents-rga` (upload/select/delete scopés par `actif_id` en premier segment du chemin), lecture via `createSignedUrl`.
- 3 blocs fonctionnels : Bloc A (fiche technique — type logement neuf/existant, date permis, upload plans surface/réseaux, upload DPE manuel), Bloc B (historique travaux — 7 types réglementaires fermés, date, description, matériaux/équipements en texte libre JSONB, justificatif upload), Bloc C (export PDF via `jsPDF`, dynamique `import("jspdf")`).
- **Hook DPE** : colonne `dpe_document_id` (UUID, non contrainte) prévue pour une future intégration d'un module DPE dédié — aujourd'hui aucun module DPE n'existe dans l'espace particulier, seul un upload manuel (`dpe_document_url`) est actif. Migration du champ vers une vraie FK à prévoir si un module DPE est créé.
- Test fonctionnel réalisé et confirmé par le PO : chargement bien, sauvegarde fiche technique, upload documents, ajout/suppression travaux, export PDF.
- **Simplification actée** : `materiaux_equipements` stocké en JSONB `{texte: "..."}` (champ libre) plutôt qu'une saisie structurée par matériau (nature/caractéristiques thermiques/surface séparés) — à faire évoluer si besoin métier plus fin identifié.
